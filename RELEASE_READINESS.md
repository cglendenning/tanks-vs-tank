...
The existing Play listing is marked removed. The Play console still shows `App removed`, `Update rejected`, and an appeal submitted September 24, 2026. The current store listing still contains the older icon/screenshots; the new source assets are committed under `marketing/screenshots/store-ready/`, but the authorized Play service account lacks access to this package and the console UI requires a manual asset upload. The accepted upload-key reset becomes valid on September 25, 2026 at 08:05:50 UTC. The production release cannot be submitted until Google reinstates the listing and the upload-key credentials are available.
